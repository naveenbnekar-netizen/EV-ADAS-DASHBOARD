...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start(&htim2);       /* counter every 1ms     */
	  /*START INTERUPPPT ON RECEIVING DATA*/
	  ...
	  while (1)
	  {
	      ...
	      if(sensor_flag == 1) //100ms tick
	      {
	    	  sensor_flag = 0;
	    	  HCSR04_ReadAll();
	    	  //update TTC ,alaram ,BSD
	    	  ADAS_Update( &adas, &ev);

	    	  //validating motor temp,soc ,collision update fault flag
...
/*UART RX byte - push to shell rong buffer,re-arm */
